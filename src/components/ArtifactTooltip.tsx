    ...
    return (
        <div key={element._id} className='text-center p-4'>
          ...
          {element.modifiers_charisma.map((modifier, index) => {
            return <p key={`char-${index}`} className="text-3xl mb-4">{modifier.attribute} + Charisma * {modifier.value}</p>
          })}
          <h1 className="text-3xl mb-4 text-darkSepia">Modifiers</h1>
          {element.modifiers.map((modifier, index) => {
            return <p key={`mod-${index}`} className="text-3xl mb-4">{modifier.attribute}: {modifier.value}</p>
          })}
