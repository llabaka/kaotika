    ...
    return (
        <div className="flex justify-around w-full">
            <button onClick={handleOnPressTicket} className="w-10 h-10 rounded-full flex justify-center items-center">
                <i className={`bx bxs-purchase-tag-alt text-2xl transition transform ${isTicketPressed ? 'scale-125 text-orange-400' : 'scale-80 text-gray-400'} hover:text-orange-400`}></i>
            </button>
            <button onClick={handleOnPressDollar} className="w-10 h-10 rounded-full flex justify-center items-center">
                <i className={`bx bx-dollar-circle text-2xl transition transform ${isDollarPressed ? 'scale-125 text-orange-400' : 'scale-80 text-gray-400'} hover:text-orange-400`}></i>
            </button>
            <button onClick={handleOnPressCart} className="w-10 h-10 rounded-full flex justify-center items-center">
                <i className={`bx bxs-cart text-2xl transition transform ${isCartPressed ? 'scale-125 text-orange-400' : 'scale-80 text-gray-400'} hover:text-orange-400`}></i>
            </button>
        </div>
    )
}
