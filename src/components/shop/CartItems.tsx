...
const CartItems: React.FC<CartItemsInterface> = ({ cartProducts, setCartProducts, handleRemoveItem, handleUpdateQuantity }) => {
  ...
  const groupedProducts = Array.from(
    cartProducts.reduce((map, item) => {
      if (map.has(item._id)) {
        const existingItem = map.get(item._id);
  ...
  return (
    <div className="h-[85%] w-[90%] text-xl overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-orange-100 [&::-webkit-scrollbar-thumb]:bg-orange-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-orange-400 pr-2">
      {groupedProducts.map((item, index) => (
        <div
          key={index}
          className="flex w-[100%] h-[23%] items-center justify-between bg-black bg-opacity-50 rounded-lg mt-2"
        ...
        >
          ...
          {/* Nombre del objeto */}
          <div
            className="flex w-[35%] h-[80%] text-3xl text-orange-300 ml-2 justify-start items-center"
            data-testid={`cart_item_${item.name}`}
          >
            {item.name}
          </div>
          ...
          <div className="flex w-[14%] h-[50%] items-center mr-2 text-right">
            <div className="w-[100%] text-right text-5xl text-orange-300 mr-1">
