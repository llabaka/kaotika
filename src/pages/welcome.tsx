...
import {Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, useDisclosure, Checkbox, Input, Link, input} from "@nextui-org/react"
...
const Welcome = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-800 text-medievalSepia">
      <h1 className="text-8xl mb-8">Welcome to Legends of Kaotika, {session?.user?.name}</h1>
      ...
      <button
        ...
        className="bg-medievalSepia w-1/4 text-black text-4xl py-2 px-4 mt-10 rounded  hover:bg-darkSepia transition"
        >
        No, continue with the one I have
      </button>
      <button
          ...
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                ...
                <KaotikaButton handleClick={handleNext} text='Save' />
              </ModalFooter>
            </>
          )}
        </ModalContent>
